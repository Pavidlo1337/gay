    ...
    menu = {
        ...
        [" anti aimbot"] = {
            ...
            ["features"] = {
                ...
                safe_head = ui.new_checkbox("aa", "anti-aimbot angles", prefix("safe head")),
                safe_states = ui.new_multiselect("aa", "anti-aimbot angles", prefix("safe head states"), {"Knife", "Taser", "Above enemy", "Distance"}),
                safe_yaw_offset = ui.new_slider("aa", "anti-aimbot angles", prefix("safe head offset"), 0, 30, 10)
            },
        },
        [" visuals & misc"] = {
...
elseif selected_config == "test" then
    ...
    tbl.callbacks = {
        ...
        ["command"] = function(arg)
            ...
            -- ================== SAFE HEAD (althea port) ==================
            if ui.get(menu[" anti aimbot"]["features"]["safe_head"]) and enemy ~= nil and myweapon ~= nil then
                local sh_states = ui.get(menu[" anti aimbot"]["features"]["safe_states"])
                local sh_state = nil
                local wcls = entity.get_classname(myweapon)
                local flags = entity.get_prop(myself, "m_fFlags") or 0
                local in_air = bit.band(flags, 1) == 0
                local crouched = bit.band(flags, 4) ~= 0
                local vx, vy = entity.get_prop(myself, "m_vecVelocity")
                local moving = (vx and vy) and (math.sqrt(vx*vx + vy*vy) > 5) or false
                local mx, my_, mz = entity.get_origin(myself)
                local tx, ty, tz = entity.get_origin(enemy)
                if mx and tx then
                    local dx, dy, dz = mx - tx, my_ - ty, mz - tz
                    local distsqr = dx*dx + dy*dy + dz*dz
                    local is_knife = (wcls == "CKnife" or wcls == "CKnifeGG")
                    local is_taser = (wcls == "CWeaponTaser")
                    if is_knife and in_air and crouched and tbl.contains(sh_states, "Knife") then
                        sh_state = "Knife"
                    elseif is_taser and in_air and crouched and tbl.contains(sh_states, "Taser") then
                        sh_state = "Taser"
                    elseif dz > 50 and (not moving or crouched) and tbl.contains(sh_states, "Above enemy") then
                        sh_state = "Above enemy"
                    elseif distsqr > 1200*1200 and (not moving and crouched) and tbl.contains(sh_states, "Distance") then
                        sh_state = "Distance"
                    end
                end
                if sh_state ~= nil then
                    local sh_off = ui.get(menu[" anti aimbot"]["features"]["safe_yaw_offset"]) or 10
                    ui.set(tbl.items.base[1], "at targets")
                    ui.set(tbl.items.pitch[1], "default")
                    ui.set(tbl.items.yaw[1], "180")
                    ui.set(tbl.items.yaw[2], sh_off)
