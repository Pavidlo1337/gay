...
do
    ...
    local function tick_target(idx)
        ...
        local fired = false
        -- ВСЕ триггеры ТРЕБУЮТ defensive контекст — фликов вне defensive НЕ бывает.
        -- Без этого детектор ловит обычные лаги/выход из dormant у всех.
        local in_def_context = was_def or s.in_def_sim

        -- (1) основной триггер: релиз пакета после defensive чока
        if in_def_context and eye_yaw and s.prev_eye_yaw and dticks >= 3 then
            local raw_delta = norm_yaw(eye_yaw - s.prev_eye_yaw)
            local abs_delta = M_abs(raw_delta)
            if abs_delta >= 75 and abs_delta <= 180 then
                local flick_dir = (raw_delta > 0) and 1 or -1
                s.flick_kill_side = -flick_dir
                s.flick_until     = tickcount + 32
                if s.flick_last_tick > 0 then
                    local p = tickcount - s.flick_last_tick
                    if p >= 4 and p <= 30 then s.flick_period = p end
                end
                s.flick_last_tick = tickcount
                fired = true
            end
        end
        -- (2) релиз defensive: was_def && !in_def_sim — сравниваем yaw с anchor
        if not fired and was_def and not s.in_def_sim and eye_yaw and anchor then
            local raw_delta = norm_yaw(eye_yaw - anchor)
            local abs_delta = M_abs(raw_delta)
            if abs_delta >= 75 then
                local flick_dir = (raw_delta > 0) and 1 or -1
                s.flick_kill_side = -flick_dir
                s.flick_until     = tickcount + 32
                s.flick_last_tick = tickcount
        ...
        -- (3) во время длинного чока: вдруг получили апдейт с большой дельтой от anchor
        if not fired and s.in_def_sim and s.choked_streak >= 3 and eye_yaw and anchor then
            local raw_delta = norm_yaw(eye_yaw - anchor)
            local abs_delta = M_abs(raw_delta)
            if abs_delta >= 75 then
                local flick_dir = (raw_delta > 0) and 1 or -1
        ...
        -- сброс по истечении lock
        if tickcount > s.flick_until and s.flick_kill_side ~= 0 then
            s.flick_kill_side = 0
            s.flick_period    = 0
        end
